...
<div class="page-header">
    ...
    <p style="color:rgba(255,255,255,0.65)">نحن نؤمن أن كل امرأة تستحق أفضل عناية</p>
</div>
...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-20">
        <div>
            ...
            <h2 class="text-3xl font-black mb-4" style="color:#1a1a1a">مركز تجميل متكامل<br>يهتم بكل تفصيلة</h2>
            <p class="leading-loose mb-4" style="color:#555">
            ...
            <p class="leading-loose" style="color:#555">
            ...
            <div class="flex flex-wrap gap-3 mt-8">
                ...
                <a href="{{ route('contact') }}" class="btn-outline" style="color:#c9888e; border-color:#e8b4b8; background:transparent">
                    تواصلي معنا
                </a>
            </div>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-5 mb-20">
        <div class="text-center p-6 rounded-2xl bg-white shadow-sm">
            <div class="text-3xl font-black mb-2" style="color:#c9888e">
                {{ $stats['clients'] > 0 ? '+' . $stats['clients'] : '+500' }}
            </div>
            <div class="text-sm" style="color:#666">عميلة راضية</div>
        </div>
        <div class="text-center p-6 rounded-2xl bg-white shadow-sm">
            <div class="text-3xl font-black mb-2" style="color:#c9888e">{{ $stats['years'] }}+</div>
            <div class="text-sm" style="color:#666">سنوات خبرة</div>
        </div>
        <div class="text-center p-6 rounded-2xl bg-white shadow-sm">
            <div class="text-3xl font-black mb-2" style="color:#c9888e">{{ $stats['services'] > 0 ? $stats['services'] . '+' : '15+' }}</div>
            <div class="text-sm" style="color:#666">خدمة متخصصة</div>
        </div>
        <div class="text-center p-6 rounded-2xl bg-white shadow-sm">
            <div class="text-3xl font-black mb-2" style="color:#c9888e">{{ $stats['rating'] }}%</div>
            <div class="text-sm" style="color:#666">ضمان الرضا</div>
        </div>
    </div>
    ...
    <div class="mb-20">
        <div class="text-center mb-12">
            ...
            <h2 class="text-3xl font-black mb-4" style="color:#1a1a1a">خبيراتنا المتميزات</h2>
            <div class="section-divider mb-4"></div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach($staff as $member)
            <div class="bg-white rounded-2xl p-6 text-center shadow-sm transition-all hover:-translate-y-1 hover:shadow-md">
                ...
                <h3 class="font-black mb-1" style="color:#1a1a1a">{{ $member->name }}</h3>
                @if($member->role)
                <p class="text-sm mb-3" style="color:#c9888e">{{ $member->role }}</p>
                @endif
                @if($member->bio)
                <p class="text-xs leading-relaxed" style="color:#888">{{ $member->bio }}</p>
                @endif
            </div>
            @endforeach
        </div>
    ...
    <div>
        <div class="text-center mb-12">
            ...
            <h2 class="text-3xl font-black mb-4" style="color:#1a1a1a">ما يميزنا عن غيرنا</h2>
            <div class="section-divider mb-4"></div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="flex items-start gap-4 bg-white rounded-2xl p-5 shadow-sm">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style="background:linear-gradient(135deg,#f5dfe1,#e8b4b8)">
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#c9888e" stroke-width="2" stroke-linecap="round">
                        <path d="{{ $feat['svg'] }}"/>
                    </svg>
                </div>
                <div>
                    <h3 class="font-black mb-1" style="color:#1a1a1a">{{ $feat['title'] }}</h3>
                    <p class="text-sm leading-relaxed" style="color:#666">{{ $feat['desc'] }}</p>
                </div>
            </div>
            @endforeach
        </div>
